...
//
// Author:           Luming Zhang, lzhang338@wisc.edu
// CS Login:         luming
...
class ProgramNode extends ASTnode {
    ...
    public Type typeChecking()
    {
	myDeclList.typeChecking();
	return null;
    }
...
class FnBodyNode extends ASTnode {
    ...
    public Type typeChecking(FnSym tmpSym)
    {
	myStmtList.typeChecking(tmpSym);
	return null;
    }
...
class StmtListNode extends ASTnode {
        ...
        while (it.hasNext()) 
	{
		...
		((ReturnStmtNode)tmp).typeChecking(tmpSym);
	    else
		tmp.typeChecking();
        }
        return null; // Place holder
...
class ExpListNode extends ASTnode {
    ...
    public Type typeChecking(FnSym sym)
    {
	Iterator<ExpNode> itExps = myExps.iterator();
	Iterator<Type> itParams = sym.getParamTypes().iterator();
	ExpNode tmpExp = null;
	Type tmpType = null;
	...
	while (itExps.hasNext()) 
        {
	    tmpExp = itExps.next();
	    tmpType = itParams.next();
	    if(tmpExp.typeChecking().equals(tmpType)==false)
	    {
	    	ErrMsg.fatal(tmpExp.lineNum(),tmpExp.charNum(),"Type of actual does not match type of formal");
		errFlag = true;
	    }
	}
	if(errFlag)
...
class FnDeclNode extends DeclNode {
    ...
    public Type typeChecking()
    {
	myBody.typeChecking((FnSym)myId.sym());
	return null;
    }
...
abstract class StmtNode extends ASTnode {
    ...
    abstract public Type typeChecking();
}
...
class PostIncStmtNode extends StmtNode {
    ...
    public Type typeChecking()
    {
	if(myExp.typeChecking().isErrorType()) return new ErrorType();
	if(myExp.typeChecking().isIntType()==false) 
	{
...
class WriteStmtNode extends StmtNode {
	...
	if(myExp.typeChecking().isVoidType())
        {
...
class IfStmtNode extends StmtNode {
    ...
    public Type typeChecking()
    {
	if(myExp.typeChecking().isErrorType()) 
	{
	    myStmtList.typeChecking();
	    return new ErrorType();
	}
        if(myExp.typeChecking().isBoolType()==false)
	{
...
class IfElseStmtNode extends StmtNode {
	...
	if(myExp.typeChecking().isErrorType()) 
        {
	    myThenStmtList.typeChecking();
            myElseStmtList.typeChecking();
            return new ErrorType();
        }
        if(myExp.typeChecking().isBoolType()==false)
        {
...
class ReturnStmtNode extends StmtNode {
    ...
    public Type typeChecking() { return null;} // Never use

    public Type typeChecking(FnSym tmp)
    {

	if (myExp != null) // Return something
	{
	    myExp.typeChecking();
	    if(tmp.getReturnType().isVoidType()) // If return type is void
	    {
	    ...
	    {
		if(tmp.getReturnType().equals(myExp.typeChecking()))
		{
		    return myExp.typeChecking();			
		}
		else
	...
	{
	    if(tmp.getReturnType().isVoidType()==false)
	    {
...
abstract class ExpNode extends ASTnode {
    ...
    public Type typeChecking(){ return null; }
...
class AssignNode extends ExpNode {
	...
	if(myExp.typeChecking().isErrorType())
	    return new ErrorType();
	if(myLhs.typeChecking().equals(myExp.typeChecking()))
	{ // Types match
            if(myLhs.typeChecking().isStructDefType())
            {
                ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(),"Struct name assignment");
                return new ErrorType();
            }

	    if(myLhs.typeChecking().isStructType())
	    {
		ErrMsg.fatal(myLhs.lineNum(), myLhs.charNum(),"Struct variable assignment");
		return new ErrorType();
	    }
	    if(myLhs.typeChecking().isFnType())
            {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeChecking()
    {
        if(myId.typeChecking().isFnType()==false) // Not a function
	{
	    ...
	    int tmp = 0;
	    if(myExpList!=null)
		tmp = myExpList.getSize();
	    if(((FnSym)myId.sym()).getNumParams()!=tmp) // Numbers mismatch
	    {
		ErrMsg.fatal(myId.lineNum(), myId.charNum(),"Function call with wrong number of args");
	    }
	    else // Numbers match, check the types of actuals
	    {
	        myExpList.typeChecking((FnSym)myId.sym());
	    }
...
class PlusNode extends BinaryExpNode {
    ...
    public Type typeChecking()
    {
	Type tmpType1 = myExp1.typeChecking();
	Type tmpType2 = myExp2.typeChecking();
	if(tmpType1.isErrorType()||tmpType2.isErrorType())
            return new ErrorType();
	if(tmpType1.isIntType()&&tmpType2.isIntType())
	    return new IntType();
	else
	{
	    if(tmpType1.isIntType()==false)
	    {
		ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),"Arithmetic operator applied to non-numeric operand");
	    }
	    if(tmpType2.isIntType()==false)
            {
...
class AndNode extends BinaryExpNode {
    ...
    public Type typeChecking()
    {
        ...
        if(tmpType1.isErrorType()||tmpType2.isErrorType())
            return new ErrorType();
        if(tmpType1.isBoolType()&&tmpType2.isBoolType())
            return new BoolType();
        else
        {
            if(tmpType1.isBoolType()==false)
            {
                ErrMsg.fatal(myExp1.lineNum(), myExp1.charNum(),"Logical operator applied to non-bool operand");
            }
            if(tmpType2.isBoolType()==false)
            {
